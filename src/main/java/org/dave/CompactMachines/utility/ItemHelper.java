...
public class ItemHelper {
    ...
    private static int findBestMatchingSlotForSidedInventory(ISidedInventory inv, ItemStack stack,
        ForgeDirection side) {
        int[] accessibleSlotsFromSide = inv.getAccessibleSlotsFromSide(side.ordinal());

        for (int slot : accessibleSlotsFromSide) {
            ItemStack target = inv.getStackInSlot(slot);
            if (target != null && target.getItem() == stack.getItem()
                && target.isStackable()
                && inv.isItemValidForSlot(slot, stack)
                && target.stackSize < target.getMaxStackSize()
                && target.stackSize < inv.getInventoryStackLimit()
                && inv.canInsertItem(slot, stack, side.ordinal())
                && (!target.getHasSubtypes() || target.getItemDamage() == stack.getItemDamage())
    ...
    private static int findFirstEmptySlotInSidedInventory(ISidedInventory inv, ItemStack stack, ForgeDirection side) {
        ...
        for (int slot : accessibleSlotsFromSide) {
            ItemStack target = inv.getStackInSlot(slot);
            if (target == null && inv.isItemValidForSlot(slot, stack)
                && inv.canInsertItem(slot, stack, side.ordinal())) {
                return slot;
            }
        }
