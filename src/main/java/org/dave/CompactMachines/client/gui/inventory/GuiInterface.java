...
public class GuiInterface extends GuiContainer {
    ...
    public void initGui() {
        ...
        hoppingButton = new GuiButton(
            0,
            xStart + (xSize / 2 - buttonWidth / 2),
            yStart + 17,
            buttonWidth,
            20,
            "Import");
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        MessageHoppingModeChange packet = new MessageHoppingModeChange(
            tileEntityInterface.coords,
            tileEntityInterface.side,
            nextHoppingMode);
    ...
    public void drawTooltips(int mouseX, int mouseY) {
        ...
        if (isPointInRegion(76, 45, 24, 16, mouseX, mouseY)) {
            String side = ForgeDirection.getOrientation(tileEntityInterface.side)
                .toString();
            ...
            if (tileEntityInterface._mana > 0) {
                ...
                lines.add(
                    String
                        .format("%s: %.1f%%", StatCollector.translateToLocal("tooltip.cm:machine.mana"), ratio * 100));
            }
                    ...
                    if (ThaumcraftApiHelper.hasDiscoveredAspect(player.getCommandSenderName(), aspect)) {
                        ...
                        lines.add(
                            StatCollector.translateToLocal("tooltip.cm:machine.unknownaspect") + ": "
                                + tileEntityInterface._aspectamount);
                    }
                }
            }
        }
