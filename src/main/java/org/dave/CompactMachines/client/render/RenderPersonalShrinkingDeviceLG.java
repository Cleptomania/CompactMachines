...
public class RenderPersonalShrinkingDeviceLG extends RenderPersonalShrinkingDevice {
    ...
    public void customRender(Tessellator tessellator, Block block, MovingObjectPosition pos) {
        ...
        if (block == ModBlocks.machine) {
            TileEntityMachine teMachine = (TileEntityMachine) Minecraft.getMinecraft().thePlayer.worldObj
                .getTileEntity(pos.blockX, pos.blockY, pos.blockZ);
            if (coordLookingAt != teMachine.coords) {
                worldview = wvapi.createWorldView(
                    ConfigurationHandler.dimensionId,
                    teMachine.getChunkCoordinates(),
                    ConfigurationHandler.psdResolutionX,
                    ConfigurationHandler.psdResolutionY);
