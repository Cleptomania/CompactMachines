...
public class ContainerMachine extends ContainerCM {
    ...
    public void detectAndSendChanges() {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    for (int i = 0; i < crafters.size(); i++) {
                        if (tank.fluid != null && tank.fluid.amount != 0) {
                            ((ICrafting) crafters.get(i))
                                .sendProgressBarUpdate(this, 40 + dir.ordinal(), tank.fluid.amount);
                            ((ICrafting) crafters.get(i))
                                .sendProgressBarUpdate(this, 50 + dir.ordinal(), tank.fluid.getFluidID());
                        } else {
                ...
                if (gasContents != null) {
                    for (int i = 0; i < crafters.size(); i++) {
                        if (gasContents.amount > 0) {
                            ((ICrafting) crafters.get(i))
                                .sendProgressBarUpdate(this, 66 + dir.ordinal(), gasContents.amount);
                    ...
                    for (int i = 0; i < crafters.size(); i++) {
                        if (aspectAmount > 0) {
                            ...
                            ((ICrafting) crafters.get(i)).sendProgressBarUpdate(
                                this,
                                87 + dir.ordinal(),
                                ThaumcraftSharedStorage.getIDForAspect(aspect));
                        } else {
            ...
            for (int i = 0; i < crafters.size(); i++) {
                ((ICrafting) crafters.get(i))
                    .sendProgressBarUpdate(this, 60 + dir.ordinal(), tileEntityMachine.getEnergyStored(dir));
            }
        }
    }
