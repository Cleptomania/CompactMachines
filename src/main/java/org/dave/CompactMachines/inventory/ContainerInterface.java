    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < crafters.size(); i++) {
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 33, tileEntityInterface.getEnergyStored(ForgeDirection.UNKNOWN));
            ((ICrafting) crafters.get(i))
                .sendProgressBarUpdate(this, 34, tileEntityInterface.getHoppingMode(ForgeDirection.UNKNOWN));
            ...
            if (Reference.THAUMCRAFT_AVAILABLE) {
                if (tileEntityInterface.getEssentiaAmount(ForgeDirection.UNKNOWN) > 0) {
                    ((ICrafting) crafters.get(i))
                        .sendProgressBarUpdate(this, 38, tileEntityInterface.getEssentiaAmount(ForgeDirection.UNKNOWN));
                    ((ICrafting) crafters.get(i)).sendProgressBarUpdate(
                        this,
                        39,
                        ThaumcraftSharedStorage
                            .getIDForAspect(tileEntityInterface.getEssentiaType(ForgeDirection.UNKNOWN)));
                } else {
