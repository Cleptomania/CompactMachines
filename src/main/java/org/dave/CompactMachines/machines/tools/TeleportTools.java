    ...
    public static void teleportPlayerToCoords(EntityPlayerMP player, int coord, boolean isReturning) {
        ...
        if (player.dimension != ConfigurationHandler.dimensionId) {
            ...
            MinecraftServer.getServer()
                ...
                .transferPlayerToDimension(player, ConfigurationHandler.dimensionId, new TeleporterCM(machineWorld));
    ...
    public static void teleportPlayerOutOfMachineDimension(EntityPlayerMP player) {
        ...
        if (cmNBT != null && cmNBT.hasKey("oldPosX")) {
            ...
            && playerNBT.getInteger("oldDimension") != ConfigurationHandler.dimensionId) {
                ...
                // In both cases the player gets trapped in a loop between the two dimensions and has no way of getting
                // back to the overworld
                // We want to allow backwards compatibility with our old settings so players in a CM during an update to
                // a version containing this commit
                // would still be trapped in the two dimensions. We can break this cycle by not allowing to get back to
                // another CM using the
                // old system.
                // That's because CM never writes its own dimension into the oldDimension tag, only RandomThings would
                // do that.
                ...
                ChunkCoordinates cc = MinecraftServer.getServer()
                    .worldServerForDimension(0).provider.getRandomizedSpawnPoint();
                targetX = cc.posX;
                targetY = cc.posY;
                targetZ = cc.posZ;
            }
        ...
        MinecraftServer.getServer()
            ...
            .transferPlayerToDimension(
                ...
                new TeleporterCM(
                    MinecraftServer.getServer()
                        .worldServerForDimension(targetDimension)));
    ...
    public static void teleportPlayerBack(EntityPlayerMP player) {
        ...
        if (playerNBT.hasKey("coordHistory")) {
            ...
            if (coordHistory.tagCount() == 0) {
                ...
                int coord = coordHistory.getCompoundTagAt(coordHistory.tagCount() - 1)
                    .getInteger("coord");
