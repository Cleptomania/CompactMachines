...
public class CubeTools {
    ...
    public static BiomeGenBase getMachineBiome(TileEntityMachine machine) {
        byte biomeArray[] = machine.getWorldObj()
            .getChunkFromBlockCoords(machine.xCoord, machine.zCoord)
            .getBiomeArray();
    ...
    public static void generateCube(TileEntityMachine machine) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            TileEntityInterface te = (TileEntityInterface) machineWorld
                .getTileEntity((int) pos.xCoord, (int) pos.yCoord, (int) pos.zCoord);
    ...
    public static void harvestMachine(TileEntityMachine machine, EntityPlayer player) {
        ...
        List<ItemStack> stacks = harvestCube(
            machineWorld,
            // x y z
            machine.coords * ConfigurationHandler.cubeDistance + 1,
            40 + 1,
            1,
            machine.coords * ConfigurationHandler.cubeDistance + size - 1,
            40 + height - 1,
            size - 1,
            player);
    ...
    private static List<ItemStack> harvestCube(World worldObj, int posX1, int posY1, int posZ1, int posX2, int posY2,
        int posZ2, EntityPlayer player) {
        int minX = Math.min(posX1, posX2);
        int minY = Math.min(posY1, posY2);
        int minZ = Math.min(posZ1, posZ2);
    ...
    public static AxisAlignedBB getBoundingBoxForCube(int coord, int size) {
        AxisAlignedBB bb = AxisAlignedBB.getBoundingBox(
            coord * ConfigurationHandler.cubeDistance + 1,
            40,
            0,
            coord * ConfigurationHandler.cubeDistance + size + 1,
            40 + size + 1,
            size + 1);
